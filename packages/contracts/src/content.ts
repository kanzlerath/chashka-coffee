...
export const contentBlockSchema = z.discriminatedUnion('type', [
  blockBase.extend({ type: z.literal('TEXT'), title: nullableText(180), text: z.string().trim().min(1).max(20_000) }).strict(),
  blockBase.extend({ type: z.literal('IMAGE'), imageUrl: publicUrl, alt: z.string().trim().min(1).max(240), caption: nullableText(300) }).strict(),
  blockBase.extend({ type: z.literal('SPLIT'), title: z.string().trim().min(1).max(180), text: z.string().trim().min(1).max(8_000), imageUrl: publicUrl, alt: z.string().trim().min(1).max(240), imagePosition: z.enum(['LEFT', 'RIGHT']) }).strict(),
  blockBase.extend({ type: z.literal('GALLERY'), images: z.array(galleryImageSchema).min(1).max(12) }).strict(),
  blockBase.extend({ type: z.literal('QUOTE'), text: z.string().trim().min(1).max(2_000), attribution: nullableText(180) }).strict(),
  blockBase.extend({ type: z.literal('VIDEO'), videoUrl: publicUrl, posterUrl: nullableUrl, title: nullableText(180) }).strict(),
  blockBase.extend({ type: z.literal('CTA'), title: z.string().trim().min(1).max(180), text: nullableText(1_000), label: z.string().trim().min(1).max(80), url: publicUrl }).strict(),
])
