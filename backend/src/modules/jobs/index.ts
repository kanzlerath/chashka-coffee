...
type JobRecord = { id: string; slug: string; title: string; department: string | null; location: string | null; employmentType: string | null; description: string | null; isPublished: boolean; status: 'DRAFT' | 'SCHEDULED' | 'PUBLISHED' | 'ARCHIVED' | null; publishAt: Date | null; position: number; createdAt: Date; updatedAt: Date }
const dto = (value: JobRecord): JobOpening => ({ id: value.id, slug: value.slug, title: value.title, department: value.department, location: value.location, employmentType: value.employmentType, description: value.description, status: value.status ?? (value.isPublished ? 'PUBLISHED' : 'DRAFT'), publishAt: value.publishAt?.toISOString() ?? null, position: value.position, createdAt: value.createdAt.toISOString(), updatedAt: value.updatedAt.toISOString() })
...
export function createJobsModule({ db, requireAuth, requireAdmin }: { db: DbClient; requireAuth: MiddlewareHandler<AuthHttpEnv>; requireAdmin: MiddlewareHandler<AuthHttpEnv> }) {
  ...
  routes.openapi(publicList, async (c) => { const now = new Date(); return c.json({ openings: (await db.jobOpening.findMany({ where: { OR: [{ status: 'PUBLISHED' }, { status: 'SCHEDULED', publishAt: { lte: now } }, { status: null, isPublished: true }] }, orderBy: [{ position: 'asc' }, { createdAt: 'desc' }] })).map(dto) }, 200) })
  routes.openapi(publicDetail, async (c) => { const now = new Date(); const opening = await db.jobOpening.findFirst({ where: { slug: c.req.valid('param').slug, OR: [{ status: 'PUBLISHED' }, { status: 'SCHEDULED', publishAt: { lte: now } }, { status: null, isPublished: true }] } }); if (!opening) throw new AppError(404, 'NOT_FOUND', 'Job opening not found'); return c.json({ opening: dto(opening) }, 200) })
  ...
  adminRoutes.openapi(create, async (c) => c.json({ opening: dto(await db.jobOpening.create({ data: data(c.req.valid('json')) })) }, 201))
  adminRoutes.openapi(update, async (c) => { const opening = await db.jobOpening.update({ where: { id: c.req.valid('param').id }, data: data(c.req.valid('json')) }); return c.json({ opening: dto(opening) }, 200) })
