...
The minimum sufficient website tier is Static Site only. The `/account` route remains a static, `noindex` shell: its private profile, balance, and QR data load in the browser from no-store backend endpoints. `webapp` remains the staff admin surface; an Astro runtime is unnecessary until a future route requires request-specific server rendering.
