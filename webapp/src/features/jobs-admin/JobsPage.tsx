...
import { adminBulkUpdateRequestSchema, adminBulkUpdateResponseSchema, jobOpeningListResponseSchema, jobOpeningResponseSchema, jobOpeningRestaurantListResponseSchema, operationSuccessResponseSchema, upsertJobOpeningRequestSchema, type JobOpening, type UpsertJobOpeningRequest } from '@chashka-coffee/contracts'
...
const empty: UpsertJobOpeningRequest = { slug: '', title: '', department: null, location: null, employmentType: null, description: null, restaurantId: null, status: 'DRAFT', publishAt: null, position: 10 }
const toDraft = (opening: JobOpening): UpsertJobOpeningRequest => { const { id: _id, restaurant, createdAt: _createdAt, updatedAt: _updatedAt, ...draft } = opening; return { ...draft, restaurantId: restaurant?.id ?? null } }
...
export function JobsPage({ mode = 'list', openingId }: { mode?: 'list' | 'create' | 'edit'; openingId?: string }) {
  ...
  const restaurants = useQuery({ queryKey: ['admin', 'jobs', 'restaurants'], queryFn: () => api.request('/api/admin/jobs/restaurants', jobOpeningRestaurantListResponseSchema) })
  ...
  const visible = useMemo(() => { const needle = query.trim().toLocaleLowerCase('ru-RU'); return openings.data?.openings.filter((opening) => (statusFilter === 'ALL' || opening.status === statusFilter) && (!needle || `${opening.title} ${opening.department ?? ''} ${opening.restaurant?.name ?? ''} ${opening.restaurant?.address ?? ''} ${opening.location ?? ''}`.toLocaleLowerCase('ru-RU').includes(needle))) ?? [] }, [openings.data, query, statusFilter])
  ...
  if (mode === 'list') return <section className="admin-page">
    ...
    <Card><CardHeader><CardTitle>Все вакансии</CardTitle></CardHeader><CardContent className="admin-directory-list">
      ...
      {visible.map((opening) => <article className="admin-selectable-row" key={opening.id}><input aria-label={`Выбрать ${opening.title}`} checked={selectedIds.includes(opening.id)} type="checkbox" onChange={() => toggle(opening.id)} /><Link className="admin-directory-row admin-directory-row-simple" to="/jobs/$openingId" params={{ openingId: opening.id }}>
        <span className="admin-directory-main"><strong>{opening.title}</strong><small>{[opening.department, opening.restaurant ? `${opening.restaurant.name} · ${opening.restaurant.address}` : opening.location, opening.employmentType].filter(Boolean).join(' · ') || 'Заведение и условия не заполнены'}</small></span>
  ...
  return <section className="admin-page admin-page-editor">
    <AdminPageHeader eyebrow="Вакансии" title={selected?.title ?? 'Новая вакансия'} description="Привяжите вакансию к заведению: оно появится в фильтре сайта, а его адрес сохранится в отклике кандидата." actions={<Button asChild variant="outline"><Link to="/jobs">К списку вакансий</Link></Button>} />
    ...
    <div className="admin-editor-layout"><Card className="admin-editor-surface"><CardHeader><CardTitle>Информация о вакансии</CardTitle><CardDescription>Эти данные увидит кандидат на сайте.</CardDescription></CardHeader><CardContent>
      <form className="admin-form-stack" id={formId} onSubmit={(event) => { event.preventDefault(); save.mutate() }}>
        ...
        <AdminField label="Название вакансии" required hint="Например: Бариста"><Input required placeholder="Бариста" value={draft.title} onChange={(event) => change('title', event.target.value)} /></AdminField>
        <AdminField label="Заведение и адрес" hint="На сайте кандидат выберет эту вакансию через заведение, а в его отклик автоматически попадут название точки и адрес."><select value={draft.restaurantId ?? ''} onChange={(event) => change('restaurantId', event.target.value || null)}><option value="">Не привязывать к заведению</option>{restaurants.data?.restaurants.map((restaurant) => <option key={restaurant.id} value={restaurant.id}>{restaurant.name} — {restaurant.address}</option>)}</select>{restaurants.isError ? <p className="admin-state-message admin-state-error">Не удалось загрузить список заведений.</p> : null}</AdminField>
        <div className="admin-form-grid-2"><AdminField label="Направление" hint="Отдел или команда, к которой относится позиция."><Input placeholder="Ресторанная команда" value={draft.department ?? ''} onChange={(event) => change('department', nullableDraftText(event.target.value))} /></AdminField><AdminField label="Дополнительное место" hint="Только для вакансий вне заведения, например: кондитерский цех."><Input placeholder="Кондитерский цех" value={draft.location ?? ''} onChange={(event) => change('location', nullableDraftText(event.target.value))} /></AdminField></div>
        ...
        <AdminField label="Задачи и детали" required hint="Коротко и по делу: что делать, какой график, опыт и условия. Это раскрывается у вакансии на сайте."><Textarea required className="min-h-56" placeholder={'• Готовить напитки по рецептурам\n• График 2/2, питание на смене\n• Опыт не обязателен'} value={draft.description ?? ''} onChange={(event) => change('description', nullableDraftText(event.target.value))} /></AdminField>
