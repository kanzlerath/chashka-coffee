...
const empty = (type: ProductType): UpsertProductRequest => ({ type, status: 'DRAFT', publishAt: null, slug: '', name: '', category: type === 'CAKE' ? 'Торты' : null, subtitle: null, description: null, ingredients: null, origin: null, roastLevel: null, tastingNotes: [], imageUrl: null, galleryUrls: [], details: [], blocks: [], isFeatured: false, position: 10, variants: [{ label: type === 'COFFEE' ? '250 г' : '1 кг', weightGrams: type === 'COFFEE' ? 250 : 1000, priceKopecks: 0, position: 10, isAvailable: true }] })
...
function ProductEditor({ type, mode, productId }: { type: ProductType; mode: 'create' | 'edit'; productId?: string }) {
  ...
  return <section className="admin-page admin-page-editor">
    ...
    <div className="admin-editor-layout"><Card className="admin-editor-surface"><CardHeader><CardTitle>Карточка товара</CardTitle><CardDescription>Название, фото и цена — минимум, необходимый для понятной карточки.</CardDescription></CardHeader><CardContent><form className="admin-form-stack" id={formId} onSubmit={(event) => { event.preventDefault(); save.mutate() }}>
      ...
      <AdminField label="Основное изображение" hint="Используется в карточке и на странице товара. После выбора можно настроить кадр для карточки."><AdminImageField cardCrop value={draft.imageUrl ?? null} onChange={(imageUrl) => change('imageUrl', imageUrl)} /></AdminField><AdminField label="Состав" hint="Ингредиенты через запятую"><Textarea placeholder="Кофе арабика 100%…" value={draft.ingredients ?? ''} onChange={(event) => change('ingredients', nullable(event.target.value))} /></AdminField>
