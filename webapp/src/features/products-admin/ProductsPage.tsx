...
function ProductEditor({ type, mode, productId }: { type: ProductType; mode: 'create' | 'edit'; productId?: string }) {
  ...
  return <section className="admin-page admin-page-editor">
    ...
    <div className="admin-editor-layout"><Card className="admin-editor-surface"><CardHeader><CardTitle>Карточка товара</CardTitle><CardDescription>Название, фото и цена — минимум, необходимый для понятной карточки.</CardDescription></CardHeader><CardContent><form className="admin-form-stack" id={formId} onSubmit={(event) => { event.preventDefault(); save.mutate() }}>
      ...
      <AdminField label="Основное изображение" hint="Используется в карточке и на странице товара. После выбора можно настроить кадр для карточки."><AdminImageField cardCrop imageCrop={draft.imageCrop} value={draft.imageUrl ?? null} onChange={(imageUrl) => { change('imageUrl', imageUrl); change('imageCrop', null) }} onImageCropChange={(imageCrop) => change('imageCrop', imageCrop)} /></AdminField><AdminField label="Состав" hint="Ингредиенты через запятую"><Textarea placeholder="Кофе арабика 100%…" value={draft.ingredients ?? ''} onChange={(event) => change('ingredients', nullable(event.target.value))} /></AdminField>
