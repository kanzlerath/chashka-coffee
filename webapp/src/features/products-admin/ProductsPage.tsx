...
function ProductList({ type }: { type: ProductType }) {
  ...
  const toggleVisible = () => setSelectedIds((ids) => allVisibleSelected ? ids.filter((id) => !visible.some((product) => product.id === id)) : [...new Set([...ids, ...visible.map((product) => product.id)])])
