...
function SlideForm({ draft, onChange, onSave, onRemove, saving, deleting, error }: { draft: UpsertHomepageSlideRequest; onChange: (value: UpsertHomepageSlideRequest) => void; onSave: () => void; onRemove?: () => void; saving: boolean; deleting: boolean; error: boolean }) {
  ...
  return <form className="admin-form-stack admin-form-subsection" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    ...
    <AdminField label={draft.mediaType === 'VIDEO' ? 'URL видео' : 'URL изображения'}><Input onChange={(event) => change('mediaUrl', event.target.value)} placeholder="/images/hero.jpg или https://…" required value={draft.mediaUrl} /></AdminField>
    {draft.mediaType === 'VIDEO' ? <AdminField label="Постер для видео"><Input onChange={(event) => change('posterUrl', nullable(event.target.value))} placeholder="/images/poster.jpg" value={draft.posterUrl ?? ''} /></AdminField> : null}
