...
import { AdminField, AdminPageHeader, AdminTabs } from '@/components/admin'
...
const emptySlide: UpsertHomepageSlideRequest = {
  mediaType: 'IMAGE', mediaUrl: '', imageFocus: null, posterUrl: null, eyebrow: null, title: '', description: null,
  ...
  return <section className="admin-page admin-homepage-workspace">
    ...
    {panel === 'slides' ? <Card>
      ...
      <CardContent className="grid gap-5">
        <div className="grid gap-1">{homepage.data?.slides.map((entry) => <button className="admin-list-row" key={entry.id} onClick={() => { setSlide(entry); setSlideDraft(toSlideDraft(entry)) }} type="button"><span><b>{entry.title}</b><small>{entry.mediaType === 'VIDEO' ? 'Видео' : 'Изображение'} · {entry.isPublished ? 'опубликован' : 'черновик'}</small></span><span className="admin-row-action">{entry.position}</span></button>)}</div>
    ...
    {panel === 'bestsellers' ? <Card>
      ...
      <CardContent className="grid gap-5">
        <div className="grid gap-1">{homepage.data?.bestsellers.map((entry) => <button className="admin-list-row" key={entry.id} onClick={() => { setBestseller(entry); setBestsellerDraft(toBestsellerDraft(entry)) }} type="button"><span><b>{entry.item.name}</b><small>{entry.badge ?? entry.item.marketingBadge ?? entry.item.categoryName} · {entry.isPublished ? 'опубликован' : 'черновик'}</small></span><span className="admin-row-action">{entry.position}</span></button>)}</div>
    ...
    {panel === 'day' ? <Card>
      ...
      <CardContent className="grid gap-5">
        ...
        {daySection ? <><div className="grid gap-1 border-t pt-5">{daySection.parts.map((entry) => <button className="admin-list-row" key={entry.id} onClick={() => { setDayPart(entry); setDayPartDraft(toDayPartDraft(entry)) }} type="button"><span><b>{entry.label} · {entry.title}</b><small>{entry.isPublished ? 'опубликован' : 'черновик'} · {entry.ctaUrl ?? 'без ссылки'}</small></span><span className="admin-row-action">{entry.position}</span></button>)}</div><Button onClick={() => { setDayPart(null); setDayPartDraft({ ...emptyDayPart(daySection.id), position: (daySection.parts.length ?? 0) * 10 + 10 }) }} type="button" variant="outline">Добавить период дня</Button><DayPartForm draft={dayPartDraft} onChange={setDayPartDraft} onRemove={dayPart ? () => removeDayPart.mutate(dayPart.id) : undefined} onSave={() => saveDayPart.mutate()} saving={saveDayPart.isPending} deleting={removeDayPart.isPending} error={saveDayPart.isError} /></> : <p className="admin-state-message">Сначала сохраните заголовок блока, затем добавьте периоды дня.</p>}
      </CardContent>
    </Card> : null}
    </div>
  </section>
...
function SlideForm({ draft, onChange, onSave, onRemove, saving, deleting, error }: { draft: UpsertHomepageSlideRequest; onChange: (value: UpsertHomepageSlideRequest) => void; onSave: () => void; onRemove?: () => void; saving: boolean; deleting: boolean; error: boolean }) {
  ...
  return <form className="admin-form-stack admin-form-subsection" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    ...
    {draft.mediaType === 'VIDEO' ? <AdminField label="Видео" hint="Загрузите MP4 в медиатеку или выберите уже загруженное видео."><AdminVideoField required value={draft.mediaUrl || null} onChange={(mediaUrl) => mediaUrl && change('mediaUrl', mediaUrl)} /></AdminField> : <AdminField label="Фотография слайда" hint="Выберите исходную фотографию, затем кликните по объекту, который должен быть в центре первого экрана." required><AdminImageField required value={draft.mediaUrl || null} imageFocus={draft.imageFocus} onChange={(mediaUrl) => { if (mediaUrl) change('mediaUrl', mediaUrl); change('imageFocus', null) }} onImageFocusChange={(imageFocus) => change('imageFocus', imageFocus)} /></AdminField>}
    ...
    <AdminField label="Короткое описание"><Textarea onChange={(event) => change('description', nullable(event.target.value))} value={draft.description ?? ''} /></AdminField>
    <div className="admin-form-grid-2"><AdminField label="Текст кнопки"><Input onChange={(event) => change('ctaLabel', nullable(event.target.value))} value={draft.ctaLabel ?? ''} /></AdminField><AdminField label="Ссылка кнопки"><Input onChange={(event) => change('ctaUrl', nullable(event.target.value))} placeholder="/restaurants" value={draft.ctaUrl ?? ''} /></AdminField></div>
    <label className="admin-check-row admin-check-row-compact"><input checked={draft.isPublished} onChange={(event) => change('isPublished', event.target.checked)} type="checkbox" />Показывать на сайте</label>
    {error ? <p className="admin-state-message admin-state-error">Не удалось сохранить слайд. Проверьте ссылку и обязательные поля.</p> : null}
    <div className="admin-form-actions"><Button disabled={saving} type="submit">{saving ? 'Сохраняем…' : 'Сохранить слайд'}</Button>{onRemove ? <Button disabled={deleting} onClick={onRemove} type="button" variant="outline">Удалить</Button> : null}</div>
  </form>
}
...
function BestsellerForm({ menuItems, draft, onChange, onSave, onRemove, saving, deleting, error }: { menuItems: HomepageBestseller['item'][]; draft: UpsertHomepageBestsellerRequest; onChange: (value: UpsertHomepageBestsellerRequest) => void; onSave: () => void; onRemove?: () => void; saving: boolean; deleting: boolean; error: boolean }) {
  ...
  return <form className="admin-form-stack admin-form-subsection" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    <AdminField label="Позиция меню"><select onChange={(event) => change('menuItemId', event.target.value)} required value={draft.menuItemId}><option value="">Выберите позицию</option>{menuItems.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.categoryName}</option>)}</select></AdminField>
    <div className="admin-form-grid-2"><AdminField label="Бейдж на главной"><Input onChange={(event) => change('badge', nullable(event.target.value))} placeholder="Хит" value={draft.badge ?? ''} /></AdminField><AdminField label="Порядок"><Input min={0} onChange={(event) => change('position', Number(event.target.value))} type="number" value={draft.position} /></AdminField></div>
    <label className="admin-check-row admin-check-row-compact"><input checked={draft.isPublished} onChange={(event) => change('isPublished', event.target.checked)} type="checkbox" />Показывать на сайте</label>
    {error ? <p className="admin-state-message admin-state-error">Не удалось сохранить позицию. Возможно, она уже добавлена в подборку.</p> : null}
    <div className="admin-form-actions"><Button disabled={saving} type="submit">{saving ? 'Сохраняем…' : 'Сохранить позицию'}</Button>{onRemove ? <Button disabled={deleting} onClick={onRemove} type="button" variant="outline">Удалить</Button> : null}</div>
  </form>
}
...
function DaySectionForm({ draft, onChange, onSave, saving, error }: { draft: UpsertHomepageDaySectionRequest; onChange: (value: UpsertHomepageDaySectionRequest) => void; onSave: () => void; saving: boolean; error: boolean }) {
  ...
  return <form className="admin-form-stack" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    <AdminField label="Заголовок слева"><Input onChange={(event) => change('title', event.target.value)} required value={draft.title} /></AdminField>
    <AdminField label="Описание слева"><Textarea onChange={(event) => change('description', nullable(event.target.value))} value={draft.description ?? ''} /></AdminField>
    <label className="admin-check-row admin-check-row-compact"><input checked={draft.isPublished} onChange={(event) => change('isPublished', event.target.checked)} type="checkbox" />Показывать на сайте</label>
    {error ? <p className="admin-state-message admin-state-error">Не удалось сохранить блок. Проверьте обязательные поля.</p> : null}
    <div className="admin-form-actions"><Button disabled={saving} type="submit">{saving ? 'Сохраняем…' : 'Сохранить блок'}</Button></div>
  </form>
}
...
function DayPartForm({ draft, onChange, onSave, onRemove, saving, deleting, error }: { draft: UpsertHomepageDayPartRequest; onChange: (value: UpsertHomepageDayPartRequest) => void; onSave: () => void; onRemove?: () => void; saving: boolean; deleting: boolean; error: boolean }) {
  ...
  return <form className="admin-form-stack admin-form-subsection" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    <div className="admin-form-grid-2"><AdminField label="Метка"><Input onChange={(event) => change('label', event.target.value)} placeholder="Утро" required value={draft.label} /></AdminField><AdminField label="Порядок"><Input min={0} onChange={(event) => change('position', Number(event.target.value))} type="number" value={draft.position} /></AdminField></div>
    <AdminField label="Заголовок"><Input onChange={(event) => change('title', event.target.value)} placeholder="Завтраки до 12:00" required value={draft.title} /></AdminField>
    <AdminField label="Короткое описание"><Textarea onChange={(event) => change('description', nullable(event.target.value))} value={draft.description ?? ''} /></AdminField>
    <AdminField label="Ссылка"><Input onChange={(event) => change('ctaUrl', nullable(event.target.value))} placeholder="/menu" value={draft.ctaUrl ?? ''} /></AdminField>
    <label className="admin-check-row admin-check-row-compact"><input checked={draft.isPublished} onChange={(event) => change('isPublished', event.target.checked)} type="checkbox" />Показывать на сайте</label>
    {error ? <p className="admin-state-message admin-state-error">Не удалось сохранить период дня. Проверьте обязательные поля.</p> : null}
    <div className="admin-form-actions"><Button disabled={saving} type="submit">{saving ? 'Сохраняем…' : 'Сохранить период'}</Button>{onRemove ? <Button disabled={deleting} onClick={onRemove} type="button" variant="outline">Удалить</Button> : null}</div>
  </form>
}
