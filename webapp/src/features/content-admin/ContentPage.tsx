import { contentEntryListResponseSchema, contentEntryResponseSchema, operationSuccessResponseSchema, type ContentEntry, type ContentEntryType, type UpsertContentEntryRequest } from '@chashka-coffee/contracts'
...
function ContentEditor({ type, mode, entryId }: { type: ContentEntryType; mode: 'create' | 'edit'; entryId?: string }) {
  ...
  return <section className="admin-page admin-page-editor"><AdminPageHeader eyebrow={meta.title} title={mode === 'create' ? `Новая ${type === 'ARTICLE' ? 'статья' : type === 'PROMOTION' ? 'акция' : 'афиша'}` : selected?.title ?? 'Редактирование'} description="Сначала сохраните черновик. Публикуйте материал только после проверки текста, фото и ссылок." actions={<Button asChild variant="outline"><Link to={meta.base}>К списку</Link></Button>} />
    <Card className="admin-editor-surface"><CardHeader><CardTitle>Содержание</CardTitle><CardDescription>Эти данные увидят гости на карточке и странице материала.</CardDescription></CardHeader><CardContent><form className="admin-form-stack" noValidate onSubmit={(event) => { event.preventDefault(); submit() }}>
      ...
      <div className="admin-form-grid-2"><AdminField label="Обложка" hint="Ссылка из медиатеки вида /images/… или полный адрес https://…"><Input inputMode="url" placeholder="/images/promotion.webp" value={draft.imageUrl ?? ''} onChange={(event) => change('imageUrl', nullable(event.target.value))} /></AdminField><AdminField label="Статус"><select value={draft.status} onChange={(event) => change('status', event.target.value as UpsertContentEntryRequest['status'])}><option value="DRAFT">Черновик — не виден на сайте</option><option value="PUBLISHED">Опубликовано</option><option value="ARCHIVED">Архив</option></select></AdminField></div>
      ...
      {save.isError ? <Typography aria-live="polite" as="p" className="admin-state-message admin-state-error" role="alert" variant="bodySm">{contentSaveErrorMessage(save.error, draft)}</Typography> : null}<div className="admin-form-actions"><Button disabled={save.isPending} size="lg" type="submit">{save.isPending ? 'Сохраняем…' : 'Сохранить материал'}</Button></div>
